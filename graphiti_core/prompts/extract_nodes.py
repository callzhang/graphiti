...
IMPORTANT: Extract mutable business concepts as entities when they act as durable, recall-worthy objects whose values change over time. Examples:
...
When a text says "目标上调到100万" or "方案从Redis改为Memcached", the target/approach should be extracted as an entity so that edges can track the changing values with timestamps.
